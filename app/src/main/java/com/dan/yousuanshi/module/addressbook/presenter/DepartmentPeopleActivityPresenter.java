...
public class DepartmentPeopleActivityPresenter extends BasePresenter<DepartmentPeopleActivityView> {
    ...
    public void addPeopleToDepartment(Map<String,Object> map){
        addToRxLife(AddressBookRequest.addPeopleToDepartment(map, new RequestListener<List>() {
            @Override
            public void onStart() {
                if(isAttach()){
                    getBaseView().showLoadingDialog();
            ...
            public void onSuccess(int code, List data) {
                if(isAttach()){
                    getBaseView().addPeopleToDepartmentSuccess(code,data);
                }
            }
            ...
            public void onFailed(int code, String msg) {
                if(isAttach()){
                    getBaseView().addPeopleToDepartmentFailed(code,msg);
                }
            }
